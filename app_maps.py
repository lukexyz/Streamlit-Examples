...
Usage: > streamlit run app_maps.py
"""
...
# Load 100,000 uber pick-ups from NYC
data = load_data(100000)

hour = st.slider("Hour to look at", 0, 23, 17)
data = data[data[DATE_TIME].dt.hour == hour]

st.subheader("Geo Data between %i:00 and %i:00" % (hour, (hour + 1) % 24))

# Mapping function (upgrade from st.map(data))
midpoint = (np.average(data["lat"]), np.average(data["lon"]))
if st.checkbox("3D Chart", True):
    st.write(pdk.Deck(
